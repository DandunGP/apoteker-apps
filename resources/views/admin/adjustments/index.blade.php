...
<div>
    ...
    <div class="breadcrumb">
        ...
        <span>&gt;</span>
        <span class="active">Stock Opname</span>
    </div>
    ...
    <div class="header-area">
        <div class="title-area">
            <h1>Stock Opname</h1>
            <p>Sesuaikan stok sistem dengan stok fisik obat di gudang secara berkala.</p>
        </div>
        <a href="{{ route('adjustments.history') }}" class="btn-action-outline">
    ...
    <div class="table-container-card">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem; padding-bottom: 1rem; border-bottom: 1px solid #F1F5F9; gap: 1rem; flex-wrap: wrap;">
            ...
            <div class="local-search-wrapper" style="position: relative; width: 280px;">
                ...
                <input type="text" id="localSearchInput" placeholder="Cari nomor batch/nama obat..." style="width: 100%; padding: 8px 12px 8px 40px !important; border: 1px solid #CBD5E1; border-radius: 4px; font-size: 12.5px; font-weight: 600; outline: none; background: #F8FAFC; box-sizing: border-box; transition: all 0.2s; height: 34px;">
        ...
        <table class="data-table" id="adjustmentsTable">
            <thead>
                <tr>
                    ...
                    <th>Obat</th>
                    <th style="width: 150px; text-align: center;">Stok Sistem</th>
            ...
            <tbody>
                @forelse($batches as $batch)
                    <tr>
                        ...
                        <td style="text-align: center; font-weight: 800; color: #0F62FE; font-size: 15px;">{{ $batch->stok_sisa }}</td>
                        <td style="font-weight: 600; color: #475569;">{{ \Carbon\Carbon::parse($batch->tanggal_kadaluwarsa)->format('d M Y') }}</td>
                        <td style="text-align: center;">
                            <a href="{{ route('adjustments.create', $batch->id) }}" class="btn-sesuaikan">
                ...
                @empty
                    <tr>
                        <td colspan="6" style="text-align: center; padding: 3rem; color: #64748B; font-weight: 600;">
                            Tidak ada batch obat aktif untuk stock opname.
                        </td>
                    </tr>
                @endforelse
            </tbody>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        let currentPage = 1;
        let rowsPerPage = 10;
        const allRows = Array.from(document.querySelectorAll("#adjustmentsTable tbody tr"));
        ...
        function applyFilters() {
            ...
            allRows.forEach(row => {
                ...
                const rowMedicine = (row.cells[2].textContent || "").toLowerCase();

                // Search Filter
                let matchesSearch = true;
                if (searchQuery !== "") {
                    matchesSearch = rowBatch.includes(searchQuery) || rowMedicine.includes(searchQuery);
                }
