...
<style>
    /* Premium clinical clinical-corporate styling for Monitoring Stok */
    ...
    .bg-icon-danger { background: #FEF2F2; color: #EF4444; }
    .bg-icon-warning { background: #EEF2F6; color: #0B3E9C; }
    .bg-icon-success { background: #ECFDF5; color: #10B981; }
    ...
    /* Pagination CSS style matches exactly the layout */
    .custom-pagination {
        display: flex;
        gap: 6px;
        align-items: center;
    ...
    .cert-card {
        background: #1E293B; /* Premium Dark Navy */
        color: white;
        border-radius: 8px;
        padding: 1.5rem;
        display: flex;
...
<div class="monitoring-container">
    ...
    <div>
        ...
        <div class="breadcrumb">
            ...
            <span>&gt;</span>
            <span class="active">Monitoring Stok</span>
        </div>
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi-card">
            <div class="kpi-left">
                <span class="kpi-title">Stok Kritis</span>
                <span class="kpi-value kpi-value-danger">{{ $metrics['stok_kritis'] }} Item</span>
            </div>
        ...
        <!-- 2. Hampir Kadaluwarsa -->
        <div class="kpi-card">
            <div class="kpi-left">
                <span class="kpi-title">Hampir Kadaluwarsa</span>
                <span class="kpi-value kpi-value-warning">{{ $metrics['hampir_expired'] }} Item</span>
            </div>
        ...
        <div class="kpi-card">
            <div class="kpi-left">
                <span class="kpi-title">Total Item Aktif</span>
                <span class="kpi-value">{{ number_format($metrics['total_aktif'], 0, ',', '.') }}</span>
            </div>
    ...
    <form action="{{ route('admin.monitoring.stock') }}" method="GET" id="stockFilterForm" class="filters-card">
        ...
        <div class="filters-left">
            ...
            <!-- Pill: Hampir Kadaluwarsa -->
            <a href="#" onclick="toggleStatusFilter('kadaluwarsa'); return false;" 
               class="filter-pill {{ $statusFilter === 'kadaluwarsa' ? 'filter-pill-active-warning' : '' }}">
        ...
        <div class="filters-right">
            ...
            <select name="sort" class="filter-select" onchange="document.getElementById('stockFilterForm').submit();">
                <option value="terendah" {{ $sortOption === 'terendah' ? 'selected' : '' }}>Urutkan Stok Terendah</option>
                <option value="tertinggi" {{ $sortOption === 'tertinggi' ? 'selected' : '' }}>Urutkan Stok Tertinggi</option>
                <option value="kadaluwarsa" {{ $sortOption === 'kadaluwarsa' ? 'selected' : '' }}>Urutkan Kadaluwarsa Terdekat</option>
            </select>
...
<script>
    ...
    function exportPDF() {
        ...
        const url = "{{ route('admin.monitoring.stock.export') }}?" + params;
        
        document.getElementById('printModalTitle').textContent = 'Preview Cetak Laporan Monitoring Stok';
    ...
    function printLabels() {
        const form = document.getElementById('stockFilterForm');
        const params = new URLSearchParams(new FormData(form)).toString();
        const url = "{{ route('admin.monitoring.stock.labels') }}?" + params;
